...
function RichCardCarouselSidebar({
  ...
  const [richCardCarousels, setRichCardCarousels] = useState({
   ...
   console.log("richCardCarousels card title",alldata?.data?.richCardCarousels?.cards[cardIndex]?.title)
  // useEffect(()=>{
  //   if(isExist?.data?.richCardCarousels?.cards){
  //     setRichCardCarousels(isExist?.data?.richCardCarousels);
  //   }
  // },[isExist]);
  ...
  const [messagename, setMessageName] = useState( alldata?.data?.richCardCarousels?.cards[cardIndex]?.title ?? "");
  ...
  const handleTemplateNameChange = (e) => {
    ...
    setRichCardCarousels((prev) => {
      const updatedRichCardCarousels = prev.map((card, index) => {
        if (index === cardIndex) {
          // Create a new card object with updated properties if it doesn't exist
          return {
            ...card,
            templateName: newTemplateName, // Update the templateName safely
          };
        }
        return card;
      });
      ...
      return updatedRichCardCarousels;
    });
  };
  ...
  //   const updatedRichCardCarousels = [...richCardCarousels];
  //   updatedRichCardCarousels[cardIndex].title = newMessageName; // Update the message description
  ...
  const handleImageUpload = (info,index,key) => {
    ...
    if (info.file.status === "done") {
      ...
      setLoading(false);
      // const updatedRichCardCarousels = [...richCardCarousels];
      // updatedRichCardCarousels[cardIndex].media = newImageUrl;// Update the card media
      setRichCardCarousels((prev) => {
        console.log("Prev State:", prev);
        console.log("Key:", key);
  ...
  const handleAddCardsTemplate = () => {
    if (options.length < 10) {
      ...
      // Update local state (options and richCardCarousels)
      setOptions((prev) => [...prev, `Card ${prev.length + 1}`]);
      setRichCardCarousels((prev) => {
        // Ensure 'prev.cards' exists and is updated correctly
        const updatedCards = [...(prev.cards || []), newCard];

        // Dispatch the updated data to Redux store
        const data = {
          selectedNode,
          value: { ...prev, cards: updatedCards }, // Keep the object structure intact
          key: "richCardCarousels",
        };
        dispatch(setRichCardNodeCarousleData(data));

        // Return the updated object with the new cards array
        return { ...prev, cards: updatedCards };
      });
    } else {
  ...
  const addNewCard = () => {
    if (richCardCarousels.cards.length < 10) {
      // Update the actions of the selected card (cardIndex)
      setRichCardCarousels((prev) => {
        const updatedCards = prev.cards.map((card, index) => {
          if (index === cardIndex) {
            const updatedActions = [
              ...card.actions,
              {
                id: card.actions.length, // Generate a new ID based on the length of the actions array
                type: "quick", // Default type
                title: "", // Empty title for the new action
                payload: "", // Empty payload for the new action
              },
            ];
        ...
        // Dispatch the updated data to Redux store
        const data = { selectedNode, value: { ...prev, cards: updatedCards }, key: "richCardCarousels" };
        dispatch(setRichCardNodeCarousleData(data));
  ...
  // const addNewCard = () => {
  //   if (richCardCarousels[cardIndex]?.actions?.length < 11) {
  //     const newAction = {
  //       id: richCardCarousels[cardIndex].actions.length,
  //       type: "quick",
  //       title: "",
  //       payload: "",
  //     };
  ...
  //     const updatedRichCardCarousels = [...richCardCarousels];
  //     updatedRichCardCarousels[cardIndex].actions.push(newAction);
  //     setRichCardCarousels(updatedRichCardCarousels);

  //     // Dispatch the updated richCardCarousels array to the Redux store
  //     const data = {
  //       selectedNode,
  //       value: updatedRichCardCarousels,
  //       key: "richCardCarousels",
  ...
  const deleteCard = (index) => {
    if (richCardCarousels?.cards[cardIndex]?.actions?.length > 1) {
      ...
      const data = { selectedNode, value: updatedRichCardCarousels, key: "richCardCarousels" };
      dispatch(setRichCardNodeCarousleData(data));
      setRichCardCarousels(updatedRichCardCarousels);
    } else {
  ...
  const handleChange = (cardIndex, index, type, value) => {
    setRichCardCarousels((prev) => {
      const updatedCards = prev.cards.map((card, i) => {
        if (i === cardIndex) {
          return {
      ...
      const data = { selectedNode, value: updatedData, key: "richCardCarousels" };
      dispatch(setRichCardNodeCarousleData(data));
      return updatedData; 
    });
  };
  ...
  //   const updatedRichCardCarousels = [...richCardCarousels];
  //   updatedRichCardCarousels[cardIndex].actions[index] = {
  //     ...updatedRichCardCarousels[cardIndex].actions[index],
  //     [key]: val,
  //   };
  //   setRichCardCarousels(updatedRichCardCarousels);

  //   // Dispatch the updated richCardCarousels
  //   const data = {
  //     selectedNode,
  //     value: updatedRichCardCarousels,
  //     key: "richCardCarousels",
        ...
        >
          <Form layout="vertical" form={form}>
            ...
            >
              <Input
                placeholder="Title"
                defaultValue={alldata?.data?.richCardCarousels?.cards[cardIndex]?.title || ""}
                id="message"
            ...
            >
              <TextArea
                ...
                rows={4}
                defaultValue={alldata?.data?.richCardCarousels?.cards[cardIndex]?.description || ""}
                // value={description}
                onChange={(e)=> handleDescriptionNameChange(e, cardIndex, "description")} // Pass cardIndex and "description
              />
            </Form.Item>
            <Row>
              <Col md={24}>
                <Form.Item
                  ...
                  initialValue={alldata?.data?.richCardCarousels?.cards[cardIndex]?.media || ""}
                >
                  <Dragger
                    {...props}
                    ...
                    onChange={(info)=>handleImageUpload(info, cardIndex, "media")}
                  >
                    {alldata?.data?.richCardCarousels?.cards[cardIndex]?.media ? (
                      <img
                        src={alldata?.data?.richCardCarousels?.cards[cardIndex]?.media}
                        alt="avatar"
                        style={{ width: "100%" }}
                      />
                    ) : (
            ...
            <Row>
              <Col md={24}>
                <Form.Item
                  ...
                  // initialValue={alldata?.data?.richCardCarousels?.cards[cardIndex]?.size || ""}
                  // initialValue={
                  //   isEdit
                ...
                >
                  <Radio.Group
                    defaultValue={alldata?.data?.richCardCarousels?.cards[cardIndex]?.size || "short"}
                  ...
                  <Col md={24}>
                    ...
                    //  name={`button-type-${index}`}
                      ...
                      <Select
                        ...
                        value={btn.type}
                        onChange={(value) => handleChange(cardIndex, index, "type", value)}
                        style={{ width: "100%" }}
                        options={[
